...
public class QuerydslBasicTest {
    ...
    public void searchAndParam() {
        ...
        Member findMember = queryFactory
                .selectFrom(member)
                .where(
                        member.username.eq("member1"), // and 일때!
                        member.age.eq(10)
                )
                .fetchOne();
...
//                .fetchFirst();
     ...
     * 1. 나이 Desc
     * 2. 이름 Asc
     * 2.1. 2에서 이름이 없으면 가장 마지막에 출력(null last)
     */
    @Test
    public void sort() {
    ...
    /**
     * 팀과 회원을 조인하면서, team 이름이 teamA인 팀만 조인, 회원은 모두 조회
     * JPQL: select m, t from Member m left join m.team t on t.name = "teamA"
     */
    // Left Join 일 경우 사실상 On 절이 의미가 있지만,
    // Inner Join 같은 경우에는 On 절은 Where 절과 같은 역할을 수행한다!!
    @Test
    ...
    public void subQuery() {
        ...
        List<Member> result = queryFactory
            ...
            // 실제 SubQuery처럼 짜는 것은 조금 어렵더라 .. Alias 충돌때문!
//        List<Member> result = queryFactory.selectFrom(member)
//                .where(member.age.eq(member.age.max()))
//                .fetch();
